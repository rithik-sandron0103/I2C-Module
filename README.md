...
- **FSM States**:
 - Master FSM - Advances on every i2c_tick:
   <p align="center"><code>IDLE -> START -> ADDR -> ADDR_ACK -> (WR_DATA -> WR_ACK or RD_DATA -> RD_ACK) -> STOP -> IDLE</code></p>
 ...
 - Slave FSM - Primarily edge-driven (Start/Stop detection + SCL edges):
   <p align="center"><code>IDLE -> ADDR -> ADDR_ACK_WAIT -> (ADDR_ACK_STRETCH ->) ADDR_ACK_RISE -> (WR_DATA -> WR_ACK_WAIT -> WR_ACK_RISE or RD_DATA -> RD_ACK_WAIT) -> WAIT_STOP -> IDLE</code></p>
...
| Feature             | Description                                                         |
|---------------------|---------------------------------------------------------------------|
| 7-bit addressing    | Standard slave address + R/W bit                                    |
| Single-byte R/W     | Write path and read path (master issues NACK to end read)           |
| Clock stretching    | Slave holds SCL low & Master freezes timing and optionally times out|
| Open-drain signaling| SCL/SDA driven low or released & pull-ups restore high              |
| START / STOP        | Properly formed with SDA transitions while SCL is high              |
| ACK / NACK          | Sampled by master; generated by slave on address and data bytes     |
...
```bash
# Compile the design
iverilog -o dsn testbench.v I2C_master.v I2C_slave.v
# Run simulation
vvp dsn
# View waveform
gtkwave i2c.vcd
```
# Waveform Analysis
**Figure 1: I²C Master Write Transaction**
<img width="1746" height="308" alt="image" src="https://github.com/user-attachments/assets/aaabd600-0fc2-416b-ae87-8205bceeb6c4" />
- System Initialization: The `clk` and active-low `arst_n` initialize the design, placing the FSM in `IDLE` with SCL and SDA high.
- Transaction Framing: A `start` pulse triggers the transfer, generating a START condition (SDA falling while SCL is high) and framing the transaction.
- Data Stream & Shifting: The master transmits the slave address (`0x50`, write) followed by data (`0xC3`), shifted MSB-first on SDA and sampled on SCL edges.
- Transaction Completion: After `ACK`s from the slave, the master issues a STOP condition (SDA rising while SCL is high) and asserts `done`, with `rx_data` correctly captured as `0xC3`.

**Figure 2: I²C Read Transaction Waveform**
<img width="1006" height="334" alt="image" src="https://github.com/user-attachments/assets/101233d4-c376-4fde-9177-f5041eff70c1" />
- Transaction Framing: A `start` pulse initiates the transfer, generating a START condition and beginning the read transaction.
- Address Phase (Read): The master transmits slave address `0x50` with `rw` = 1. The slave acknowledges by pulling SDA low (`ACK`).
- Data Stream & Sampling: The slave drives SDA with data `0x7E`, shifted MSB-first. The master samples each bit on SCL edges, assembling the byte into `data_out`.
- Transaction Completion: After receiving the byte, the master issues a `NACK` to end the read, followed by a STOP condition. The `done` signal asserts, and `data_out` = `0x7E` confirms correct reception.

**Figure 3: **
<img width="1737" height="303" alt="image" src="https://github.com/user-attachments/assets/a5292f2d-ce17-468c-a91d-12b958c81452" />
- Transaction Framing: A `start` pulse generates the START condition, beginning the write attempt to a non-existent slave.
- Address Phase (Mismatch): The master transmits slave address `0x11` with `rw` = 0. No slave responds to this address.
- NACK Detection: During the `ACK` cycle, SDA remains high, indicating a `NACK`. The master samples this and asserts `nack_flag` = 1.
- Transaction Completion: The master aborts the transfer, issues a STOP condition, and asserts `done` without proceeding to data transfer.

# Results
- Correct parameterized I²C master–slave communication verified for both write and read transactions
- Accurate START/STOP generation and SDA stability maintained while SCL is high
- Reliable ACK/NACK handling for valid transfers and address-mismatch scenarios
- Clock stretching successfully handled with master-side timing freeze and recovery
- Proper MSB-first data shifting and sampling validated across all transaction phases
- Robust open-drain bus behavior confirmed with shared SCL/SDA signaling
